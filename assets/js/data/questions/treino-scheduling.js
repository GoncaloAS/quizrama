...
const Q_TREINO_SCHEDULING = [
  ...
  [
    ...
    1,
    "<b>(B) 2.67 ✓ correcta:</b> SJF ordena por burst crescente: P2(2) → P1(4) → P3(8).<br>• WT(P2) = 0<br>• WT(P1) = 2<br>• WT(P3) = 6<br>Média: (0 + 2 + 6) / 3 = 8/3 ≈ <b>2.67</b>.\n\n<b>(A) ✗</b>: 3.33 é a média do FCFS — comparação útil!\n<b>(C) ✗</b>: 5.00 não corresponde.\n<b>(D) ✗</b>: 6.00 é apenas a maior espera.",
  ...
  [
    ...
    1,
    "<b>(B) 3.67 ✓ correcta:</b> ordem por prioridade (1 &lt; 2 &lt; 3): P2(prio=1) → P1(prio=2) → P3(prio=3).<br>• WT(P2) = 0<br>• WT(P1) = 3 (espera P2)<br>• WT(P3) = 8 (espera P2 + P1)<br>Média: (0 + 3 + 8) / 3 = 11/3 ≈ <b>3.67</b>.\n\n<b>(A), (C), (D) ✗</b>: outros valores não correspondem.",
  ...
  [
    ...
    1,
    "<b>(B) ✓ correcta:</b> convoy effect = jobs curtos a aguardar atrás de jobs longos numa fila FCFS, fazendo subir o waiting time médio drasticamente. Exemplo clássico: P_longo(burst=100) chega antes de 10 jobs curtos (burst=1 cada) → todos esperam ~100s.\n\n<b>(A) ✗</b>: chegadas simultâneas não causam convoy por si só.\n<b>(C) ✗</b>: monopolização é outro problema.\n<b>(D) ✗</b>: troca rápida = problema de overhead, não de convoy.",
  ...
  [
    ...
    1,
    "<b>(B) ✓ correcta:</b> calculemos:<br>• <b>SJF</b> (P2→P3→P1): WT = (0+1+3)/3 = 1.33.<br>• <b>FCFS P1→P2→P3</b>: WT = (0+3+4)/3 = <b>2.33</b>.<br>• <b>FCFS P2→P3→P1</b>: WT = (0+1+3)/3 = 1.33 (= SJF).<br>• <b>RR Q=1</b>: P1[0-1] P2[1-2] P3[2-3] P1[3-4] P3[4-5] P1[5-6]. CT: P1=6, P2=2, P3=5. WT = (3+1+3)/3 = 2.33.<br>Empate B vs D, mas a opção B (FCFS pior ordem) é a resposta clássica do \"convoy effect\".\n\n<b>(A) ✗</b>: SJF é óptimo (menor WT).\n<b>(C) ✗</b>: ordem boa do FCFS coincide com SJF.\n<b>(D) ✗</b>: RR Q=1 dá igual neste caso.",
