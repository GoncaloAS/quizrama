...
const Q_EXAME_PARTE_V = [
  [
    ...
    2,
    "<b>(C) ✓ correcta:</b> se a CS é mais curta que o tempo de troca de contexto, o overhead de suspender/acordar (mutex) supera o de simplesmente fazer busy-wait. Spinlock é mais eficiente.\n\n<b>(B) ✗</b>: se a CS é maior que o tempo de espera, busy-wait queima muito CPU.\n\n<b>(A) ✗</b>: invertido — esse cenário favorece semáforo com fila.",
  ...
  [
    ...
    2,
    "<b>(C) ✓ correcta:</b> tem um bug evidente (<code>p1</code> repõe <code>c2 = 1</code> em vez de <code>c1 = 1</code>) e mesmo sem o bug a estrutura não garante exclusão mútua nem progresso (ambos podem ficar bloqueados ou entrar simultaneamente).\n\n<b>(A) ✗</b>: o código não está correcto.\n\n<b>(B) ✗</b>: a correcção pode ser determinada estaticamente — não depende do código da CS.",
  ...
  [
    ...
    3,
    "<b>(D) ✓ correcta:</b> <i>readers-preference</i> — enquanto houver pelo menos um leitor, <code>bd</code> está ocupado e os escritores não podem entrar. Pode causar starvation dos escritores.\n\n<b>(B) ✗</b>: prioridade não é dos escritores.\n\n<b>(A) ✗</b>: opposite.\n\n<b>(C) ✗</b>: leitores têm vantagem clara.",
